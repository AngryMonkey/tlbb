...
function x402077_OnDefaultEvent( sceneId, selfId, targetId )
	...
	BeginEvent(sceneId)
		...
		AddNumText( sceneId, x402077_g_ScriptId, "挑战", 10, 1 )

		--判断当前是否可以挑战李秋水....	
--		if 1 == CallScriptFunction( x402077_g_FuBenScriptId, "GetBossBattleFlag", sceneId, "LiQiuShui" ) then
--			AddNumText( sceneId, x402077_g_ScriptId, "决战李秋水？", 10, 2 )
--		end
...
--		CallScriptFunction( x402077_g_FuBenScriptId, "OpenPMFTimer", sceneId, 7, x402077_g_LossScriptId, -1 ,-1 )
		...
		--建立BOSS....
		CallScriptFunction( x402077_g_FuBenScriptId, "TipAllHuman", sceneId, "战斗开始" )
		--删除NPC....
...
function x402077_OnPMFTimer( sceneId, step, data1, data2 )
	...
	if 7 == step then
		CallScriptFunction( x402077_g_FuBenScriptId, "TipAllHuman", sceneId, "战斗5秒钟后开始" )
		return
	end

	if 6 == step then
		CallScriptFunction( x402077_g_FuBenScriptId, "TipAllHuman", sceneId, "战斗4秒钟后开始" )
		return
	end

	if 5 == step then
		CallScriptFunction( x402077_g_FuBenScriptId, "TipAllHuman", sceneId, "战斗3秒钟后开始" )
		return
	end

	if 4 == step then
		CallScriptFunction( x402077_g_FuBenScriptId, "TipAllHuman", sceneId, "战斗2秒钟后开始" )
		return
	end

	if 3 == step then
		CallScriptFunction( x402077_g_FuBenScriptId, "TipAllHuman", sceneId, "战斗1秒钟后开始" )
		return
	end
