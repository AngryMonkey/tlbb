...
--任务是否已经完成
x250026_g_IsMissionOkFail = 0		--变量的第0位

x250026_g_PetDataID=3250	--任务珍兽的编号
--以上是动态**************************************************************

--任务变量第一位用来存储随机得到的脚本号

--任务文本描述
x250026_g_MissionName="捕捉珍兽"
x250026_g_MissionInfo="我家小姐想要一只孔雀。"  --任务描述
x250026_g_MissionTarget="帮云涵儿找到一只孔雀。"		--任务目标
x250026_g_ContinueInfo="你捉到孔雀了吗？"		--未完成任务的npc对话
x250026_g_MissionComplete="干的不错。"					--完成任务npc说话的话
...
--任务入口函数
--**********************************
function x250026_OnDefaultEvent( sceneId, selfId, targetId )	--点击该任务后执行此脚本
	--如果已接此任务
...
function x250026_OnAccept( sceneId, selfId )
  ...
  if( 0 == x250026_GetTimePass( sceneId, selfId ) ) then      
	...
	misIndex = GetMissionIndexByID(sceneId,selfId,x250026_g_MissionId)			--得到任务的序列号
	SetMissionByIndex(sceneId,selfId,misIndex,0,0)						--根据序列号把任务变量的第0位置0 (任务完成情况)
	SetMissionByIndex(sceneId,selfId,misIndex,1,x250026_g_ScriptId)						--根据序列号把任务变量的第1位置为任务脚本号
...
--杀死怪物或玩家
--**********************************
function x250026_OnKillObject( sceneId, selfId, objdataId ,objId)--参数意思：场景号、玩家objId、怪物表位置号、怪物objId
end
	...
	if indexpet == 255 then --索引值返回255表示空，没提交珍兽
		...
		if petdataid == x250026_g_PetDataID then
			--奖励金钱并提示
				...
				BeginEvent(sceneId)
					AddText(sceneId,"恭喜你完成了任务，奖励你#{_MONEY"..x250026_g_MoneyBonus.."}")
				EndEvent(sceneId)
				DispatchEventList(sceneId,selfId,npcid)
			ret0 = DelMission( sceneId, selfId, x250026_g_MissionId ) --删除任务
