...
function x890071_OnDefaultEvent( sceneId, selfId, targetId )
	...
	BeginEvent(sceneId)
		AddText( sceneId, "鸠摩智真是没有。我可不会放过你们的。" )
		AddNumText( sceneId, x890071_g_ScriptId, "挑战", 10, 1 )
	EndEvent(sceneId)
...
function x890071_OnPMFTimer( sceneId, step, data1, data2 )
	...
	if 7 == step then
		CallScriptFunction( x890071_g_FuBenScriptId, "TipAllHuman", sceneId, "战斗5秒钟后开始" )
		return
	end

	if 6 == step then
		CallScriptFunction( x890071_g_FuBenScriptId, "TipAllHuman", sceneId, "战斗4秒钟后开始" )
		return
	end

	if 5 == step then
		CallScriptFunction( x890071_g_FuBenScriptId, "TipAllHuman", sceneId, "战斗3秒钟后开始" )
		return
	end

	if 4 == step then
		CallScriptFunction( x890071_g_FuBenScriptId, "TipAllHuman", sceneId, "战斗2秒钟后开始" )
		return
	end

	if 3 == step then
		CallScriptFunction( x890071_g_FuBenScriptId, "TipAllHuman", sceneId, "战斗1秒钟后开始" )
		return
	end

	if 2 == step then
		--提示战斗开始....
		CallScriptFunction( x890071_g_FuBenScriptId, "TipAllHuman", sceneId, "战斗开始" )
		--删除NPC....
