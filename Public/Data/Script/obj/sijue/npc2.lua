...
function x910002_OnDefaultEvent( sceneId, selfId, targetId )
	...
	BeginEvent(sceneId)
		...
		AddNumText( sceneId, x910002_g_ScriptId, "挑战", 10, 1 )
	EndEvent(sceneId)
...
--	end
		...
		if ( lev>  110 ) then
		--建立BOSS....
		CallScriptFunction( x910002_g_FuBenScriptId, "TipAllHuman", sceneId, "战斗开始" )
		--删除NPC....
...
function x910002_OnPMFTimer( sceneId, step, data1, data2 )
	...
	if 6 == step then
		CallScriptFunction( x910002_g_FuBenScriptId, "TipAllHuman", sceneId, "战斗5秒钟后开始" )
		return
	end

	if 5 == step then
		CallScriptFunction( x910002_g_FuBenScriptId, "TipAllHuman", sceneId, "战斗4秒钟后开始" )
		return
	end

	if 4 == step then
		CallScriptFunction( x910002_g_FuBenScriptId, "TipAllHuman", sceneId, "战斗3秒钟后开始" )
		return
	end

	if 3 == step then
		CallScriptFunction( x910002_g_FuBenScriptId, "TipAllHuman", sceneId, "战斗2秒钟后开始" )
		return
	end

	if 2 == step then
		CallScriptFunction( x910002_g_FuBenScriptId, "TipAllHuman", sceneId, "战斗1秒钟后开始" )
		return
	end
