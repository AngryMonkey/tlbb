...
function x899996_OnDefaultEvent( sceneId, selfId, targetId )
		...
		if lev < 10 then
			...
			BeginEvent(sceneId)
				...
				AddNumText( sceneId, x899996_g_scriptId, "#c00ff00传送至盘龙阁", 5, 200)
			EndEvent(sceneId)
			DispatchEventList(sceneId,selfId,targetId)
		end
end
...
function x899996_OnEventRequest( sceneId, selfId, targetId, eventId )
	...
	for i=0, nCount-1  do
		...
		if MosDataID ==  14120 or MosDataID == 14121 or MosDataID == 14122 or MosDataID == 14123 or MosDataID == 14124 or MosDataID == 14125 then
              	        BeginEvent( sceneId ) 
	        	           AddText( sceneId, "#G西韵阁怪物还没有消灭干净，请等待全部消灭之后再来传送！如果你确定消灭干净了，让我检查一下，30秒后再找我传送！" )
              	           EndEvent( sceneId )
               	        DispatchEventList( sceneId, selfId, targetId )
		  return
		else
		...
		if lev < 1 then
			...
			DispatchEventList(sceneId,selfId,targetId)
			         ...
			         CallScriptFunction(899992 , "MakeCopyScene", sceneId, selfId)--传送
			         EndEvent( sceneId )
               	         DispatchEventList( sceneId, selfId, targetId )
