...
--buff....
x910005_Buff_MianYi1	= 10472	--免疫一些负面效果....
x910005_Buff_MianYi2	= 10471	--免疫普通隐身....
...
--AI Index....
x910005_IDX_CombatTime		= 1	--进入战斗的计时器....用于记录已经进入战斗多长时间了....
x910005_IDX_UseSkillIndex	= 2	--接下来该使用技能表中的第几个技能....

x910005_IDX_CombatFlag 			= 1	--是否处于战斗状态的标志....
x910005_IDX_IsKuangBaoMode	= 2	--是否处于狂暴模式的标志....
...
--	CallScriptFunction( x910005_g_FuBenScriptId, "SetBossBattleFlag", sceneId, "HaDaBa", 2 )
	...
	if playerName ~= nil then
		str = format("#{_INFOUSR%s}#P在四绝庄#P中与#Y[闵墨]#P坚强对峙，一记耳光，连番天马流星拳......#Y[闵墨]#P终抵不住，惨死在四绝殿之上。", playerName); --哈大霸
...
function x910005_UseMySkill( sceneId, selfId, skill )
	...
	elseif skill == "B" then
		...
		MonsterTalk(sceneId, -1, "", "#ccc9933[闵墨]#W哎呦，哎呦，腿好疼..." )
	...
	elseif skill == "C" then
		...
		MonsterTalk(sceneId, -1, "", "#ccc9933[闵墨]#W不行了，头疼脑胀的..." )
		local x,z = GetWorldPos( sceneId, selfId )
	...
	elseif skill == "D" then
		...
		if enemyId > 0 then
			...
			MonsterTalk(sceneId, -1, "", "#ccc9933[闵墨]#W我开始生气了，后果很严重..." )
			local x,z = GetWorldPos( sceneId, selfId )
...
function x910005_SkillA_NaLiZou( sceneId, selfId )
	...
	MonsterTalk(sceneId, -1, "", "#ccc9933[闵墨]#W看我的定身术..." )
