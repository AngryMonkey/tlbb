...
function x891000_OnPlayerPickUpItemInLuoyang( sceneId, selfId, itemId, bagidx )
	...
	local message
	if rand == 1 then
		message = format("@*;SrvMsg;SCA:#P生死门内风起云涌。#{_INFOUSR%s}#P经过自己努力，终于将#{_INFOMSG%s}#P抢入手中。最终获得宝盒者将获得100W元宝和高级宝石、终极神器奖励,大家可以在洛阳生死夺宝NPC进入!", playerName, transfer )
	elseif rand == 2 then
		message = format("@*;SrvMsg;SCA:#P号外号外。#{_INFOUSR%s}#P在生死门手脚快速的将#{_INFOMSG%s}#P捡入囊中后低调的转身而去。最终获得宝盒者将获得100W元宝和高级宝石、终极神器奖励,大家可以在洛阳生死夺宝NPC进入!", playerName, transfer )
	else
		message = format("@*;SrvMsg;SCA:#P轰隆隆，天空一声雷鸣！#{_INFOUSR%s}#P在生死门内终于把#{_INFOMSG%s}#P得到手中。最终获得宝盒者将获得100W元宝和高级宝石、终极神器奖励,大家可以在洛阳生死夺宝NPC进入!", playerName, transfer )
	end
