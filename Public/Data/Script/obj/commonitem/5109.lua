...
--脚本号
 x335109_g_scriptId = 335109 --临时写这个,真正用的时候一定要改.
...
--注：两个骑术会一个就可以骑了
 x335109_g_Equitation1 = 1140 --需要的骑术
 x335109_g_Equitation2 = -1 --需要的骑术
--效果的ID
 x335109_g_Impact1 = 4765 --临时写这个
 x335109_g_Impact2 = -1 --不用

--**********************************
--事件交互入口
--**********************************
function  x335109_OnDefaultEvent( sceneId, selfId, bagIndex )
-- 骑乘不需要这个接口，但要保留空函数
end
...
function  x335109_CancelImpacts( sceneId, selfId )
	nRet = 0;
	nRet = LuaFnCancelSpecificImpact(sceneId, selfId,  x335109_g_Impact1)
	if(0<nRet) then
		return 1;
	end
	return 0;
...
function  x335109_OnConditionCheck( sceneId, selfId )
	...
	--end
	--两个骑术，会一个就可以了
	if(-1~= x335109_g_Equitation1) then
		if(1==LuaFnHaveSpecificEquitation(sceneId, selfId,  x335109_g_Equitation1)) then
			return 1
		end
	end
	if(-1~= x335109_g_Equitation2) then
		if(1==LuaFnHaveSpecificEquitation(sceneId, selfId,  x335109_g_Equitation2)) then
			return 1
		end
	end
...
function  x335109_OnActivateOnce( sceneId, selfId )
	if(-1~= x335109_g_Impact1) then
		LuaFnSendSpecificImpactToUnit(sceneId, selfId, selfId, selfId,  x335109_g_Impact1, 0)
	end
