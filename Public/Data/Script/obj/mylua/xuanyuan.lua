...
--解决流失率兑换门派套装事件脚本
x889064_g_MenPaiTaoScriptId = 500617
...
function x889064_UpdateEventList( sceneId, selfId,targetId )
	BeginEvent(sceneId)
		...
		for i, eventId in x889064_g_eventList do
			CallScriptFunction( eventId, "OnEnumerate",sceneId, selfId, targetId )
		end

		AddText( sceneId, "           #e0e8de5#Y夺宝#W轩辕幻羽" )
		...
		AddNumText( sceneId, x889064_g_ScriptId, "#G轩辕令#W兑换#cFF0000灵魂幻羽", 4, 1000 )
		--AddNumText( sceneId, x889064_g_ScriptId, "#G灵魂碎片#W兑换#cFF0000高级属性武魂", 0, 2000 )
		--AddNumText( sceneId, x889064_g_ScriptId, "#G怪物相册#W兑换#cFF0000高级属性时装", 0, 3000 )
		--AddNumText( sceneId, x889064_g_ScriptId, "#G七夕花瓣#W兑换#cFF0000顶级暗器材料", 0, 4000 )
		--AddNumText( sceneId, x889064_g_ScriptId, "#G古书残页#W兑换#cFF0000120级门派技能", 0, 5000 )
		--AddNumText( sceneId, x889064_g_ScriptId, "兑换70级套装", 0, 6000 )
		--AddNumText( sceneId, x889064_g_ScriptId, "兑换80级套装", 0, 7000 )
		--AddNumText( sceneId, x889064_g_ScriptId, "兑换90级套装", 0, 8000 )
		--AddNumText( sceneId, x889064_g_MenPaiTaoScriptId, "兑换师门套装", 0, 846 )
		
		-- 褚少微，2008.5.29。极品装备放出。
		--AddNumText( sceneId, x889064_g_ScriptId, "魔兵天降", 0, 9000 )
		--AddNumText( sceneId, x889064_g_ScriptId, "魔兵天降介绍", 11, 10000 )

		AddNumText( sceneId, x889064_g_ScriptId, "我要参加镜湖夺宝", 4, 200 )
...
function x889064_OnEventRequest( sceneId, selfId, targetId, eventId )
	...
	if eventId == x889064_g_MenPaiTaoScriptId then
		...
		BeginEvent(sceneId)
			AddText(sceneId, "  #G请选择您兑换的物品：")
				if nNumText < 2000  then
					AddNumText(sceneId, x889064_g_ScriptId, "#c009933兑换轩辕幻羽", 4, nNumText+100)
					AddNumText(sceneId, x889064_g_ScriptId, "#c009933轩辕幻羽仙术", 4, nNumText+200)
				end
			if nNumText >= 2000 and nNumText < 3000 then
			AddNumText(sceneId, x889064_g_ScriptId, "#cFF0000御瑶盘（火）", 0, nNumText+100)
			AddNumText(sceneId, x889064_g_ScriptId, "#W御瑶盘（冰）", 0, nNumText+200)
			AddNumText(sceneId, x889064_g_ScriptId, "#Y御瑶盘（玄）", 0, nNumText+300)
			AddNumText(sceneId, x889064_g_ScriptId, "#c009933御瑶盘（毒）", 0, nNumText+400)
			end
			if nNumText >= 3000 and nNumText < 4000  then
					AddNumText(sceneId, x889064_g_ScriptId, "兑换#G高级属性时装#cFF0000①", 0, nNumText+100)
					AddNumText(sceneId, x889064_g_ScriptId, "兑换#G高级属性时装#cFF0000②", 0, nNumText+200)
				end
			if nNumText >= 4000 and nNumText < 5000  then
					AddNumText(sceneId, x889064_g_ScriptId, "#G兑换#cFF0000顶级暗器", 0, nNumText+100)
					AddNumText(sceneId, x889064_g_ScriptId, "#G兑换#cFF0000顶级武魂", 0, nNumText+200)
				end
			if nNumText >= 5000 then
					AddNumText(sceneId, x889064_g_ScriptId, "#G兑换#cFF0000内攻门派", 0, nNumText+100)
					AddNumText(sceneId, x889064_g_ScriptId, "#G兑换#cFF0000外攻门派", 0, nNumText+200)
				end
			AddNumText( sceneId, x889064_g_ScriptId, "离开……", 0, 0 )
		EndEvent(sceneId)
		DispatchEventList(sceneId,selfId,targetId)
	...
	if nNumText > 1000 and nNumText < 9000  then
		BeginEvent(sceneId)
			...
			local szStr = "  #{JPZB_0610_04}" .. x889064_g_StoneList[nLevel].str 
										.. "”“".. tostring(x889064_g_StoneList[nLevel].num) .. "#{JPZB_0610_05}"
			AddText(sceneId, szStr)
			
			for i, item in x889064_g_EquipList do
				if item.n == nNumText  then
					AddRadioItemBonus( sceneId, item.id, 4 )
				end
			end
    EndEvent(sceneId)
    --DispatchMissionDemandInfo(sceneId,selfId,targetId, x889064_g_ScriptId, x210200_g_MissionId)
    DispatchMissionContinueInfo(sceneId,selfId,targetId, x889064_g_ScriptId, 0)
		
	end
	
	for i, findId in x889064_g_eventList do
...
--接受此NPC的任务
--**********************************
function x889064_OnMissionAccept( sceneId, selfId, targetId, missionScriptId )
	for i, findId in x889064_g_eventList do
...
--拒绝此NPC的任务
--**********************************
function x889064_OnMissionRefuse( sceneId, selfId, targetId, missionScriptId )
	--拒绝之后，要返回NPC的事件列表
	for i, findId in x889064_g_eventList do
		if missionScriptId == findId then
			x889064_UpdateEventList( sceneId, selfId, targetId )
...
--继续（已经接了任务）
--**********************************
function x889064_OnMissionContinue( sceneId, selfId, targetId, missionScriptId )
	for i, findId in x889064_g_eventList do
		if missionScriptId == findId then
...
--提交已做完的任务
--**********************************
function x889064_OnMissionSubmit( sceneId, selfId, targetId, missionScriptId, selectRadioId )

	--处理提交后的显示情况
	--为了安全，这里要仔细，不能出错
	local nItemIndex = -1
	
	if missionScriptId == x889064_g_MenPaiTaoScriptId then
	...
	-- 看完家是不是够材料提交
	local nLevel = 0
	if x889064_g_EquipList[nItemIndex].n > 1000 then
		nLevel = 1
	end
	if x889064_g_EquipList[nItemIndex].n > 1100 then
		nLevel = 2
	end
	if x889064_g_EquipList[nItemIndex].n > 2000 then
		nLevel = 2
	end
	if x889064_g_EquipList[nItemIndex].n > 2100 then
		nLevel = 3
	end
	if x889064_g_EquipList[nItemIndex].n > 2200 then
		nLevel = 4
	end
	if x889064_g_EquipList[nItemIndex].n > 2300 then
		nLevel = 5
	end
	if x889064_g_EquipList[nItemIndex].n > 3000 then
		nLevel = 6
	end
	if x889064_g_EquipList[nItemIndex].n > 4000 then
		nLevel = 7
	end
	if x889064_g_EquipList[nItemIndex].n > 4100 then
		nLevel = 8
	end
	if x889064_g_EquipList[nItemIndex].n > 5000 then
		nLevel = 9
	end
	if x889064_g_EquipList[nItemIndex].n > 6000 then
		nLevel = 6
	end
	if x889064_g_EquipList[nItemIndex].n > 7000 then
		nLevel = 7
	end
	if x889064_g_EquipList[nItemIndex].n > 8000 then
		nLevel = 8
	end
	
	-- 褚少微，2008.5.29。极品装备放出。
	if x889064_g_EquipList[nItemIndex].n == 9100 then
	...
	local bStoneOk = 0
	if GetItemCount(sceneId, selfId, x889064_g_StoneList[nLevel].id) >= x889064_g_StoneList[nLevel].num  then
		bStoneOk = 1
	end
	...
	-- 检查是不是有足够的石头可以扣除
	if LuaFnGetAvailableItemCount(sceneId, selfId, x889064_g_StoneList[nLevel].id) < x889064_g_StoneList[nLevel].num   then
		BeginEvent(sceneId)
			-- 褚少微，2008.5.29。极品装备放出。
			if nLevel == 9 then
				strText = "#{JPZB_0610_07}"
	...
	local nItemBagIndexStone = GetBagPosByItemSn(sceneId, selfId, x889064_g_StoneList[nLevel].id)
	local szTransferStone = GetBagItemTransfer(sceneId,selfId, nItemBagIndexStone)
	
	-- 删除相关的石头
	local bDelOk = LuaFnDelAvailableItem(sceneId,selfId, x889064_g_StoneList[nLevel].id, x889064_g_StoneList[nLevel].num)
	...
	if bDelOk < 1  then
		...
		--
		local nBagIndex = TryRecieveItem( sceneId, selfId, x889064_g_EquipList[nItemIndex].id, 1 );
		...
		local sItemName = GetItemName(sceneId, x889064_g_EquipList[nItemIndex].id)
...
--死亡事件
--**********************************
function x889064_OnDie( sceneId, selfId, killerId )
end